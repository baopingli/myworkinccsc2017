function [ d_res, z_res, Dz, obj_val ] = ResBal_Z_admm_learn(b, kernel_size,...
...
%         tic;
        ...
        if i_z>1
            ...
            dua_res = rho_z * norm(reshape(real(ifft2(reshape(permute(dTdeltU1hat_blocks, [3,2,1]), size_z)))...
                + (u2_old-u_Z{2}),[],1));
            pri_res = sqrt(norm(reshape(v_Z{1}-u_Z{1},[],1))^2 + norm(reshape(v_Z{2}-u_Z{2},[],1))^2);
