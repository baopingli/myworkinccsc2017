function [ d_res, z_res, Dz, obj_val ] = ResBal_DandZ_admm_learn(b, kernel_size,...
...
lambda = [lambda_residual, lambda_prior]; % weights for data fedelity and spasity items respectively.
gamma_heuristic = 60 * lambda_prior * 1/max(b(:));% gamma_heuristic =  lambda_prior * 1/max(b(:));% 
gammas_D = [gamma_heuristic / 5000, gamma_heuristic];% gammas_D = [gamma_heuristic, gamma_heuristic];% gammas_D = [gamma_heuristic / 5000, gamma_heuristic];% penalty factors for {Dz, z} respectively.
gammas_Z = [gamma_heuristic / 5000, gamma_heuristic];% gammas_Z = [gamma_heuristic, gamma_heuristic];%
Mu=10;
tau=2;
...
dTdeltU1hat_blocks=zeros(size_z(3),size_z(4),size_z(2)*size_z(1));%20x5x106x106
...
%         tic;
        ...
        if i_d>1
            deltU1_blocks = permute( reshape(fft2(u1_old-u_D{1}), size_x(1)*size_x(2), size_x(3), size_x(4)), [3,2,1] );
             for ii=1:size_x(1)*size_x(2)
                zTdeltU1hat_blocks(:,:,ii) =  zhatT_blocks(:,:,ii) * deltU1_blocks(:,:,ii);
             end
            dua_res_d = rho_d * norm(reshape(real(ifft2(reshape(permute(zTdeltU1hat_blocks, [3,2,1]), size_k_full)))...
                + (u2_old-u_D{2}),[],1));
            pri_res_d = sqrt(norm(reshape(v_D{1}-u_D{1},[],1))^2 + norm(reshape(v_D{2}-u_D{2},[],1))^2);
            
            if pri_res_d > Mu*dua_res_d
                rho_d = min(tau * rho_d_pre, rho_d_MAX);
                rho_d_pre = rho_d;
            elseif dua_res_d > Mu*pri_res_d
        ...
        for c = 1:2
            ...
            d_D{c} = d_D{c}*(rho_d_pre/rho_d) - (v_D{c} - u_D{c});
...
%         t_kernel = t_kernel + t_kernel_tmp;
        ...
        obj_val = objective(z, d_hat);
        if strcmp(verbose, 'brief') || strcmp( verbose, 'all')
            data_dua_res_d(end+1)=dua_res_d;
            data_pri_res_d(end+1)=pri_res_d;
            data_rho_d(end+1)=rho_d;
            fprintf('--> Obj %5.5g \n', obj_val )
        end
    end
...
%         tic;
        ...
        if i_z>1
            %��ʱDz
            deltU1_blocks=permute(reshape(fft2(u1_old-u_Z{1}),size_x(1)*size_x(2),size_x(3),size_x(4)),[2,3,1]);
            for ii=1:size_x(1)*size_x(2)
                dTdeltU1hat_blocks(:,:,ii) = dhatT_blocks(:,:,ii)*deltU1_blocks(:,:,ii);
            end
            dua_res_z = rho_z * norm(reshape(real(ifft2(reshape(permute(dTdeltU1hat_blocks, [3,2,1]), size_z)))...
                + (u2_old-u_Z{2}),[],1));
            pri_res_z = sqrt(norm(reshape(v_Z{1}-u_Z{1},[],1))^2 + norm(reshape(v_Z{2}-u_Z{2},[],1))^2);
            if pri_res_z>Mu*dua_res_z
                rho_z=min(tau*rho_z_pre,rho_z_MAX);
                rho_z_pre=rho_z;
            elseif dua_res_z>Mu*pri_res_z
                rho_z=max(1/tau*rho_z_pre,rho_z_MIN);
                rho_z_pre=rho_z;
            end
        end
        ...
        for c = 1:2
            ...
            d_Z{c} = d_Z{c}*(rho_z_pre/rho_z) - (v_Z{c} - u_Z{c});
...
%         t_vars = t_vars + t_vars_tmp;
        ...
        obj_val = objective(z, d_hat);
        if strcmp(verbose, 'brief') || strcmp( verbose, 'all')
            data_pri_res_z(end+1)=pri_res_z;
            data_dua_res_z(end+1)=dua_res_z;
            data_rho_z(end+1)=rho_z;
            fprintf('--> Obj %5.5g \n', obj_val )
        end
...
save ./data_pri_res_z.mat data_pri_res_z;
save ./data_dua_res_z.mat data_dua_res_z;
save ./data_dua_res_d.mat data_dua_res_d;
save ./data_pri_res_d.mat data_pri_res_d;
save ./data_obj_D.mat data_obj_D;
save ./data_obj_Z.mat data_obj_Z;
save ./data_rho_z.mat data_rho_z;
save ./data_rho_d.mat data_rho_d;
